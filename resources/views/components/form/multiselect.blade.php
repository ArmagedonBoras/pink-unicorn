<div class="form-group {{ $divClass ?? '' }}">
    ...
    <select id="{{ $name }}" name="{{ $name }}"
        class="form-select{{ $errors->has($name) ? ' is-invalid' : '' }} {{ $inputClass }}"
        ...
        @foreach ((array) $options as $key => $tag)
            @if (is_array($tag))
                ...
                @foreach ((array) $tag as $itemkey => $item)
                    <option value="{{ $itemkey }}" {{ old($name, $value) == $itemkey ? 'selected' : '' }}>
                        {{ $item }}</option>
                @endforeach
            @else
                <option value="{{ $key }}"
                    {{ in_array($key, old($name, $value->toArray())) ? 'selected' : '' }}>
                    {{ $tag }}</option>
            @endif
        @endforeach
    </select>
